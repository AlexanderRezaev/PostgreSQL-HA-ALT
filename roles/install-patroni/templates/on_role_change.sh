...
if [[ "$2" == "master" ]]; then
sudo /usr/bin/ip address add ${ICFGADDR}/${ICFGMASK} dev ${ICFGDEV} >>${PATRONI_LOG} 2>&1 && echo "cluster ip added" >>${PATRONI_LOG} 2>&1
...
sudo /usr/bin/systemctl restart pgagent >>${PATRONI_LOG} 2>&1
{% endif %}
...
{% if install_pgagent|default(false)|bool == true %}
sudo /usr/bin/systemctl stop pgagent >>${PATRONI_LOG} 2>&1
{% endif %}
{% if install_pgbouncer|default(false)|bool == true %}
sudo /usr/bin/systemctl kill pgbouncer >>${PATRONI_LOG} 2>&1
{% endif %}
{% if install_pgpool|default(false)|bool == true and postgresql_version|int < 14 %}
sudo /usr/bin/systemctl stop pgpool >>${PATRONI_LOG} 2>&1
{% endif %}
{% if install_pgpool|default(false)|bool == true and postgresql_version|int >= 14 %}
sudo /usr/bin/systemctl stop pgpool >>${PATRONI_LOG} 2>&1
{% endif %}
