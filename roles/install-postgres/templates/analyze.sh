...
IFS="|"
echo "===== $(date '+%Y-%m-%d %H:%M:%S') ANALYZE started on port ${port} ====="  > ${logfile} 2>&1
...
echo "  $(date '+%Y-%m-%d %H:%M:%S') : ANALYZE VERBOSE \"${schemaName}\".\"${tableName}\";" >> ${logfile} 2>&1
psql -p ${port} -d ${DBs[$dbName]} -c "ANALYZE VERBOSE \"${schemaName}\".\"${tableName}\";" >> ${logfile} 2>&1
done;
done;
done;
echo "===== $(date '+%Y-%m-%d %H:%M:%S') ANALYZE finished on port ${port} =====" >> ${logfile} 2>&1
exit 0
